...
class UserServiceTest {
    ...
    void setUp() {
        ...
        when(roleRepository.(eq("Admin"))).thenReturn(Optional.of(adminRole));
        when(roleRepository.findAll()).thenReturn(List.of(adminRole@@)); // Asegura que siempre devuelva la lista correcta
    ...
    void testRegistrarUsuario_Exitoso() {
        ...
        verify(roleRepository, times(1)).findByNombreIgnoreCase(eq("Admin"));  // 🔥 Asegurar que se llamó correctamente
    }
